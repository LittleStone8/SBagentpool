...
class xicidownloader(Downloader):
    ...
    def checkip(targeturl,ip):
        ...
        try:
            response=requests.get(url=targeturl, proxies=proxies, headers=headers, timeout=xicicommon.checkip_max_quest_timeout).status_code
            if response == 200 :
                return True
            else:
                return False
